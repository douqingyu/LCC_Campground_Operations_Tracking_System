...
def add_comment(issue_id):
    ...
    When a Helper or Admin comments on a new, stalled, or resolved issue,
    it should automatically be set to open.
    """
    ...
    with db.get_cursor() as cursor:
        ...
        if session['role'] in ['helper', 'admin'] and issue['status'] in ['new', 'stalled', 'resolved']:
            cursor.execute('''
                UPDATE issues
                SET status = 'open'
                WHERE issue_id = %s
